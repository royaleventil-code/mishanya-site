...
for (const age of ages) {
  for (const gender of ["boy", "girl"]) {
    ...
    const imagePath = `public/og/${gender}-${age}-whatsapp-preview.png`;
    const htmlPath = `out/ru/${gender}/${age}.html`;
    const ageLabel = childAgeLabel(age);
    const expectedImageUrl = `${siteUrl}/og/${gender}-${age}-whatsapp-preview.png`;
...
const homeImagePath = "public/og/home-whatsapp-preview.png";
const homeImageUrl = `${siteUrl}/og/home-whatsapp-preview.png`;
const homeTitle = `${siteName} — детские праздники в Израиле`;

const homeImageSize = pngSize(homeImagePath);
