...
const TRUST = [
  { icon: Camera, label: "10 000+ праздников" },
  { icon: ShieldCheck, label: "11 лет на сцене" },
  { icon: Palette, label: "Свой реквизит и костюмы" },
  { icon: CalendarCheck, label: "Бронь даты за минуту" },
  { icon: Languages, label: "Ведём на русском и иврите" },
  { icon: BadgeCheck, label: "Любим то, что делаем" },
];
    ...
    <main className="min-h-screen overflow-x-hidden bg-[#fffaf4] text-[var(--color-ink)]">
      ...
      <section className="px-5 pt-14 sm:px-6 sm:pt-20">
        <div className="mx-auto max-w-6xl">
          ...
          <h1 className="mt-3 max-w-4xl break-words font-[family-name:var(--font-nunito)] text-[34px] font-black leading-tight tracking-tight sm:text-6xl">
            Реальные праздники и счастливые дети.
          </h1>
          <p className="mt-4 max-w-2xl text-base leading-7 text-[var(--color-ink-soft)] sm:text-lg">
            Не верьте на слово — смотрите сами. Живые фото с мероприятий, видео шоу-программ и
            сотни отзывов родителей, которые возвращаются к нам год за годом.
          </p>
      ...
      <section className="px-5 py-14 sm:px-6 sm:py-20">
        <div className="mx-auto max-w-6xl">
          ...
          <h2 className="mt-2 font-[family-name:var(--font-nunito)] text-[28px] font-black leading-tight tracking-tight sm:text-4xl">
            Фото с наших праздников
          </h2>
          <p className="mt-3 max-w-2xl text-base leading-7 text-[var(--color-ink-soft)] sm:text-lg">
            Дни рождения, шоу-программы, ростовые куклы, неон, мыльные пузыри и любимые герои —
            для малышей, мальчишек и принцесс любого возраста.
          </p>
      ...
      <section id="reviews" className="bg-white px-5 py-14 sm:px-6 sm:py-20">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1">
            ...
            <span className="text-base font-bold text-zinc-900">5,0 · 783 отзыва</span>
            <span className="text-sm text-[var(--color-ink-soft)]">— настоящие отзывы из Facebook</span>
          </div>

          <div className="hide-scrollbar mt-8 flex gap-3 overflow-x-auto pb-2 sm:gap-4">
            {REVIEWS.map((item, i) => (
              <ReviewTile key={item.src} item={item} motionProps={card(i)} />
            ))}
          </div>
      ...
      <section className="px-5 py-14 sm:px-6 sm:py-20">
        <div className="mx-auto max-w-6xl">
          ...
          <h2 className="mt-2 font-[family-name:var(--font-nunito)] text-[28px] font-black leading-tight tracking-tight sm:text-4xl">
            Смотрите праздники в движении
          </h2>
          <p className="mt-3 max-w-2xl text-base leading-7 text-[var(--color-ink-soft)] sm:text-lg">
            Нажмите на превью, чтобы открыть ролик на YouTube — шоу трансформеров, фокусник,
            принцессы, Маша и Медведь и другие любимые программы.
          </p>

          <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4">
            {MEDIA.map((item, i) => (
              <MediaTile key={item.src} item={item} motionProps={card(i)} />
            ))}
          </div>
        ...
        <div className="relative mx-auto max-w-6xl overflow-hidden rounded-[32px] bg-[#0f0f14] px-6 py-14 text-center sm:px-10 sm:py-20">
          ...
          >
            ...
            <h2 className="mx-auto mt-3 max-w-2xl font-[family-name:var(--font-nunito)] text-[30px] font-black leading-tight tracking-tight text-white sm:text-5xl">
              Хотите такой же праздник для вашего ребёнка?
            </h2>
...
function ReviewTile({ item, motionProps }: { item: ProofLinkImage; motionProps: MotionProps }) {
  ...
  if (wholeLink) {
    return (
      <a
        ...
        target="_blank"
        rel="noreferrer"
        aria-label={wholeLink.label}
        title={wholeLink.label}
...
function MediaTile({ item, motionProps }: { item: ProofLinkImage; motionProps: MotionProps }) {
  const activeHotspots = item.hotspots?.filter((h) => h.href) ?? [];
