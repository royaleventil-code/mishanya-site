...
function faqJsonLd(locale: Locale) {
  ...
  const copy = FAQ_PAGE[locale];
  const pageUrl = siteUrl(`/${locale}/faq`);
...
export default async function FaqPage({ params }: Props) {
  ...
  const waHref = whatsappLink(waMessages.default);
  // экранируем «<»: текст с "</script>" не должен ломать inline-скрипт
  const jsonLd = JSON.stringify(faqJsonLd(locale)).replace(/</g, "\\u003c");
      ...
      <div className="mx-auto max-w-3xl px-5 pb-20 pt-6 sm:px-6">
        ...
        <article className="mt-4 overflow-hidden rounded-[28px] bg-white p-5 shadow-[0_16px_40px_rgba(15,15,20,0.06)] sm:p-7">
          ...
          <p className="mt-3 text-[15px] leading-relaxed text-[var(--color-ink-soft)]">
            <BidiText locale={locale}>{copy.intro}</BidiText>
          </p>
          ...
          <section className="mt-8 border-t border-black/5 pt-5">
            <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1.5 text-sm font-semibold">
              <Link
              ...
              {CITIES.filter((city) => hasCityCopy(locale, city.id)).map((city) => (
                <Link
                  ...
                  className="text-[var(--color-ink-soft)] underline-offset-4 transition hover:underline"
                >
